                    ...
                    <div class="list-group list-group-flush" id="contact-list">
                        @if($users->isEmpty())
                            <div class="text-center p-4 text-muted">
                                ...
                                <p class="mb-0">No user contacts available</p>
                            </div>
                        @else
                            @foreach($users as $user)
                                <a href="{{ route('vet.messages.index', ['user' => $user->id]) }}" 
                ...
                <div class="card-body d-flex flex-column">
                    ...
                    @else
                        <div class="text-center my-5">
                            ...
                            <p class="text-muted">Choose a user from the contacts list to begin a conversation.</p>
                        </div>
                    @endif
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateContactUnreadCount(contactId, count) {
        ...
        if (count !== null) {
            ...
            fetch(`{{ url('/vet/messages/contact-unread-count') }}?contact_id=${contactId}`)
    ...
    function markMessagesAsRead(contactId) {
        fetch('{{ route("vet.messages.mark-as-read") }}', {
            method: 'POST',
            headers: {
