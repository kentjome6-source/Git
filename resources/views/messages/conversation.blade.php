                                ...
                                @foreach($messages as $message)
                                    <div class="mb-3 {{ $message->sender_id == Auth::id() ? 'text-end' : 'text-start' }} message" data-message-id="{{ $message->id }}">
                                        <div class="d-inline-block p-3 rounded-3 shadow-sm 
                                            {{ $message->sender_id == Auth::id() ? 'text-white' : 'bg-light' }}" 
                                            style="max-width: 80%; word-wrap: break-word; {{ $message->sender_id == Auth::id() ? 'background-color: #5b4b9b;' : '' }}">
                                            {{ $message->message }}
                ...
                data.messages.forEach(msg => {
                    ...
                    if (!existingMessage) {
                        ...
                        messageContent.className = msg.is_sender ? 
                            'd-inline-block p-3 rounded-3 shadow-sm text-white' : 
                            'd-inline-block p-3 rounded-3 shadow-sm bg-light';
                        if (msg.is_sender) {
                            messageContent.style.backgroundColor = '#5b4b9b';
                        }
