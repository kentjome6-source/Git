            ...
            <div class="row mb-4 mx-2 mx-md-0">
                <div class="col-12">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div>
                            <h2 class="mb-1" id="page-heading">Pets Available for Adoption</h2>
                            <p class="text-muted mb-0">Find your new furry family member</p>
                        </div>
                        <div class="d-flex flex-wrap gap-2 adoption-buttons">
                            <a href="{{ route('adoptions.history') }}" class="btn btn-outline-primary adoption-history-btn" role="button">
                                <i class="fas fa-history me-2" aria-hidden="true"></i>My Adoption History
                            </a>
                            <a href="{{ route('adoptions.create') }}" class="btn btn-primary list-pet-btn" role="button" aria-label="List a Pet for Adoption">
                    ...
                    <div class="card border-0 shadow-sm h-100 adoption-card" role="article" aria-labelledby="pet-name-{{ $adoption->id }}">
                        ...
                        <div class="card-body d-flex flex-column">
                            <div class="d-flex justify-content-between align-items-start">
                                <h5 class="card-title text-dark mb-0">{{ $adoption->pet_name }}</h5>
                                @if($adoption->uploader_type === 'vet')
                                    <span class="bg-success bg-opacity-10 text-success text-xs fw-semibold px-2 py-1 rounded ms-2">
                                        Vet Upload
                                    </span>
                                @endif
                            </div>
                            ...
                            <div class="d-flex flex-column flex-md-row gap-2 mt-auto">
                                <a href="{{ route('adoptions.show', $adoption) }}" class="btn btn-success flex-fill" role="button" aria-label="View details for {{ $adoption->pet_name }}">
                                ...
                                @if($adoption->user_id != auth()->id())
                                    @if($adoption->isAvailable())
                                        <form action="{{ route('adoptions.adopt', $adoption) }}" method="POST" class="d-inline">
                                            ...
                                            <button type="submit" class="btn btn-primary flex-fill" 
                                                ...
                                                <i class="fas fa-paw me-1" aria-hidden="true"></i>Adopt Pet
                                            </button>
                                        </form>
                                    @elseif($adoption->hasPendingRequest() && $adoption->pendingRequest()->adopter_id == auth()->id())
                                        <button class="btn btn-warning flex-fill" disabled>
                                            <i class="fas fa-clock me-1" aria-hidden="true"></i>Pending Approval
                                        </button>
                                    @elseif($adoption->hasApprovedRequest() && $adoption->adoptionRequests()->where('status', 'approved')->where('adopter_id', auth()->id())->exists())
                                        <button class="btn btn-success flex-fill" disabled>
                                            <i class="fas fa-check me-1" aria-hidden="true"></i>Approved - Complete Adoption
                                        </button>
                                    @elseif($adoption->hasApprovedRequest())
                                        <button class="btn btn-secondary flex-fill" disabled>
                                            <i class="fas fa-ban me-1" aria-hidden="true"></i>Adoption Approved
                                        </button>
                                    @elseif($adoption->is_adopted)
                                        <button class="btn btn-secondary flex-fill" disabled>
                                            <i class="fas fa-ban me-1" aria-hidden="true"></i>Already Adopted
                                        </button>
                                    @else
                                        <button class="btn btn-secondary flex-fill" disabled>
                                            <i class="fas fa-ban me-1" aria-hidden="true"></i>Requested by Another
                                        </button>
                                    @endif
                                @else
                                    <!-- Owner-specific buttons -->
                                    @if($adoption->hasPendingRequest())
                                        <a href="{{ route('adoptions.show', $adoption) }}" class="btn btn-info flex-fill" role="button">
...
@media (min-width: 768px) {
    ...
    /* Adopt Pet button size reduction */
