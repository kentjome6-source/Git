...
@section('title', 'Edit ' . ucfirst($lostFound->type) . ' Pet Listing')
...
@section('content')
        <div class="page-header">
            <h1 class="page-title">Edit {{ ucfirst($lostFound->type) }} Pet Listing</h1>
            <p class="page-subtitle">Update your pet listing information</p>
        </div>
        ...
        <div class="form-container">
            ...
            <form action="{{ route('lost-found.update', $lostFound) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="pet_name" name="pet_name" class="form-input" 
                           value="{{ old('pet_name', $lostFound->pet_name) }}" placeholder="Enter pet's name" required>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select id="pet_type" name="pet_type" class="form-select" required>
                            ...
                            <option value="dog" {{ old('pet_type', $lostFound->pet_type) == 'dog' ? 'selected' : '' }}>Dog</option>
                            <option value="cat" {{ old('pet_type', $lostFound->pet_type) == 'cat' ? 'selected' : '' }}>Cat</option>
                            <option value="bird" {{ old('pet_type', $lostFound->pet_type) == 'bird' ? 'selected' : '' }}>Bird</option>
                            <option value="rabbit" {{ old('pet_type', $lostFound->pet_type) == 'rabbit' ? 'selected' : '' }}>Rabbit</option>
                            <option value="hamster" {{ old('pet_type', $lostFound->pet_type) == 'hamster' ? 'selected' : '' }}>Hamster</option>
                            <option value="fish" {{ old('pet_type', $lostFound->pet_type) == 'fish' ? 'selected' : '' }}>Fish</option>
                            <option value="other" {{ old('pet_type', $lostFound->pet_type) == 'other' ? 'selected' : '' }}>Other</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="breed" class="form-label">Breed</label>
                        <input type="text" id="breed" name="breed" class="form-input" 
                               value="{{ old('breed', $lostFound->breed) }}" placeholder="e.g., Golden Retriever">
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" id="color" name="color" class="form-input" 
                               value="{{ old('color', $lostFound->color) }}" placeholder="e.g., Brown, White, Black">
                    </div>
                    <div class="form-group">
                        <label for="size" class="form-label">Size</label>
                        <select id="size" name="size" class="form-select">
                            <option value="">Select size</option>
                            <option value="small" {{ old('size', $lostFound->size) == 'small' ? 'selected' : '' }}>Small</option>
                            <option value="medium" {{ old('size', $lostFound->size) == 'medium' ? 'selected' : '' }}>Medium</option>
                            <option value="large" {{ old('size', $lostFound->size) == 'large' ? 'selected' : '' }}>Large</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" id="age" name="age" class="form-input" 
                               value="{{ old('age', $lostFound->age) }}" min="0" max="30" placeholder="Age in years">
                    </div>
                    <div class="form-group">
                        ...
                        <div class="radio-group">
                            <div class="radio-item">
                                <input type="radio" name="gender" value="male" id="gender_male" 
                                       {{ old('gender', $lostFound->gender) == 'male' ? 'checked' : '' }}>
                                <label for="gender_male">Male</label>
                            </div>
                            <div class="radio-item">
                                <input type="radio" name="gender" value="female" id="gender_female"
                                       {{ old('gender', $lostFound->gender) == 'female' ? 'checked' : '' }}>
                                <label for="gender_female">Female</label>
                            </div>
                            <div class="radio-item">
                                <input type="radio" name="gender" value="unknown" id="gender_unknown"
                                       {{ old('gender', $lostFound->gender) == 'unknown' ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" name="description" class="form-textarea" 
                              placeholder="Describe the pet's appearance, behavior, and any distinctive features..." required>{{ old('description', $lostFound->description) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="location" name="location" class="form-input" 
                           value="{{ old('location', $lostFound->location) }}" placeholder="Where was the pet lost/found?" required>
                ...
                <div class="form-group">
                    ...
                    <input type="hidden" id="latitude" name="latitude" value="{{ old('latitude', $lostFound->latitude) }}">
                    <input type="hidden" id="longitude" name="longitude" value="{{ old('longitude', $lostFound->longitude) }}">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" id="date_lost_found" name="date_lost_found" class="form-input" 
                           value="{{ old('date_lost_found', $lostFound->date_lost_found->format('Y-m-d')) }}" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="contact_name" name="contact_name" class="form-input" 
                           value="{{ old('contact_name', $lostFound->contact_name) }}" required>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="tel" id="contact_phone" name="contact_phone" class="form-input" 
                               value="{{ old('contact_phone', $lostFound->contact_phone) }}" placeholder="Your phone number" required>
                    </div>
                    <div class="form-group">
                        <label for="contact_email" class="form-label">Email</label>
                        <input type="email" id="contact_email" name="contact_email" class="form-input" 
                               value="{{ old('contact_email', $lostFound->contact_email) }}" placeholder="Your email address">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    @if($lostFound->image_path)
                        <div class="file-preview">
                            <p>Current photo:</p>
                            <img src="{{ asset('storage/' . $lostFound->image_path) }}" alt="Current pet photo">
                        </div>
                    @endif
            ...
            <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee;">
                <form action="{{ route('lost-found.destroy', $lostFound) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this listing? This action cannot be undone.')">
                    @csrf
                    @method('DELETE')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const map = L.map('map').setView([8.3450, 125.9800], 13);
            ...
            // If we have existing coordinates, set marker
